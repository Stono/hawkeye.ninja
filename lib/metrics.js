...
module.exports = function Metrics(config) {
  ...
  self.incrementCounter = function(name, value, done) {
    const counter = new Counter({ id: name, redis: config.redis });
    counter.inc(value, done);
  };

  self.getCounter = function(name, done) {
    const counter = new Counter({ id: name, redis: config.redis });
    counter.get(done);
  };

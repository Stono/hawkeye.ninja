...
const ViewModelBuilder = function(options) {
  ...
  self.validateToken = function() {
    addToQueue('validateToken', [], next => {
      const result = req.params.token.match(/^[a-z0-9]{96}$/);
      const valid = !util.isEmpty(result);
  ...
  self.validateScanNumber = function() {
    addToQueue('validateScanNumber', [], next => {
      const valid = util.isInt(req.params.scanNumber);
      if(!valid) { return next(new Error('Invalid Scan Number')); }
      req.params.scanNumber = parseInt(req.params.scanNumber);
      next();
    });
    return self;
  };
  ...
  self.loadUserFromTracking = function() {
    addToQueue('loadUser', ['loadTracking'], next => {
      if(util.isEmpty(model.tracking)) {
        return next(new Error('No tracking user found in request'));
      }
      userStore.get(model.tracking.user, (err, user) => {
        if(err) { return next(err); }
        if(util.isEmpty(user)) { return next(new Error('No user found with id: ' + model.tracking.user)); }
        model.user = user.profile;
        model.oauth = user.oauth;
        next();
      });
    });
    return self;
  ...
  self.loadTracking = function() {
    self.validateToken();
    addToQueue('loadTracking', ['validateToken'], next => {
  ...
  self.withScan = function() {
    self.validateScanNumber();
    addToQueue('withScan', ['loadScans', 'validateScanNumber'], next => {
      model.scan = model.scans.find(scan => {
        return scan.number === req.params.scanNumber;
      });

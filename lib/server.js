...
module.exports = function Server(options) {
  ...
  (function applyRoutes() {
    const middleware = {
      ...
      tokenThrottle: new Throttler({ dal: options.dal }).middleware({ key: 'params.token', amount: 2, per: 30 }),
      repoThrottle: new Throttler({ dal: options.dal }).middleware({
        key: ['params.org', 'params.repo'],
        amount: 2,
        per: 30
      }),

    ...
    const handleSocketConnection = socket => {
      const handleStreamLogsRequest = data => {
        ...
        if(util.isEmpty(viewModel.repo)) {
          debug('Socket connected, but repo not matched', socket.params);
        ...
        const forEachScan = (err, scans) => {
          ...
          if(util.isEmpty(viewModel.scan)) {
            debug('Socket connected, but scan number not matched', socket.params);
            return socket.disconnect();
          }
          ...
          log.all(parseInt(viewModel.repo.id), parseInt(scan), forEachLogItem);
        };
        scanManager.scans(viewModel.repo.id, forEachScan);
      };

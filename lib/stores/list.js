...
module.exports = function List(options) {
  ...
  self.fromEnd = function(amount, done) {
    options.redis.llen(options.id, (err, length) => {
      options.redis.lrange(options.id, length-1, length-(amount-1), (err, items) => {
        if(err) { return done(err); }
  ...
  self.fromStart = function(amount, done) {
    options.redis.lrange(options.id, 0, amount-1, (err, items) => {
      if(err) { return done(err); }
      done(null, items.map(decode));
    });
  };

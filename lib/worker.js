...
module.exports = function Worker(config) {
  ...
  const handlePending = (err, model) => {
    ...
    const log = new ScanLog({ dal: config.dal });

    log.write(model.repo.id, model.scan.number, 'Request being handled by: ' + self.id);
    log.write(model.repo.id, model.scan.number, 'Cloning ' + model.repo.fullName);
    ...
    exec.command(command, {
      ...
      log.write(model.repo.id, model.scan.number, 'Request complete');
    });
  };

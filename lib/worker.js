...
module.exports = function Worker(config) {
  util.enforceArgs(config, ['encryptedRedis']);
  ...
  const handlePending = (err, model) => {
    ...
    const scanManager = new ScanManager({ encryptedRedis: config.encryptedRedis, id: model.repo.id });
  ...
  let timer;
  self.start = function() {
    const scanManager = new ScanManager({ encryptedRedis: config.encryptedRedis, id: 0 });
    const pop = function() {
      if(working) { return; }

...
module.exports = function ScanLog(options) {
  util.enforceArgs(options, ['repoId', 'number', 'encryptedRedis']);
  let self = {};
  const namespace = `scans:${options.repoId}:log:${options.number}`;
  const list = new List({ id: namespace, redis: options.encryptedRedis });
  ...
  self.write = function(msgs, done) {
    ...
    msgs.forEach(msg => {
      options.encryptedRedis.publish(namespace, encode(msg));
      list.push(msg, done);
    });
  };

...
module.exports = function ScanManager(options) {
  ...
  let update = (number, status, output, done) => {
    const updateMetrics = next => { metrics.update(output, next); };
    let scanMetrics;
  ...
  self.handleResult = function(result, done) {
    incrementScans((err, data) => {
      if(err) { return done(err); }
      update(data.number, result.metadata.state, result.results, done);
    });
  };

  self.handleScan = function(id, result, done) {
console.log(id, result);
    update(id, result.metadata.state, result.results, done);
  };
